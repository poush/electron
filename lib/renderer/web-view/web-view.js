...
class WebViewImpl {
  ...
  attachGuestInstance (guestInstanceId) {
    ...
    this.webContents = remote ? remote.getGuestWebContents(this.guestInstanceId) : {}
...
const registerWebViewElement = function () {
  ...
  const makeError = function (method) {
    return Error(`Cannot call ${method} because the webContents is unavailable. The WebView must be attached to the DOM and the dom-ready event emitted before this method can be called.`)
  }
  ...
  const createBlockHandler = function (method) {
    return function (...args) {
      ...
      const [error, result] = ipcRenderer.sendSync('ELECTRON_BROWSER_SYNC_CALL_TO_GUEST_VIEW', internal.guestInstanceId, method, ...args)
      if (error) {
  ...
  const createNonBlockHandler = function (method) {
    return function (...args) {
      ...
      ipcRenderer.send('ELECTRON_BROWSER_ASYNC_CALL_TO_GUEST_VIEW', null, internal.guestInstanceId, method, ...args)
    }
  }
  ...
  const createCallbackHandler = function (method) {
    return function (...args) {
      const internal = v8Util.getHiddenValue(this, 'internal')
      if (!internal.webContents) {
        throw makeError(method)
      ...
      ipcRenderer.send('ELECTRON_BROWSER_ASYNC_CALL_TO_GUEST_VIEW', requestId, internal.guestInstanceId, method, ...args)
  ...
  for (const method of callbackMethods) {
    proto[method] = createCallbackHandler(method)
  }

  // WebContents associated with this webview.
  const getWebContents = function () {
    const internal = v8Util.getHiddenValue(this, 'internal')
    if (!internal.webContents) {
  ...
  if (remote) {
    proto.getWebContents = getWebContents
  }
